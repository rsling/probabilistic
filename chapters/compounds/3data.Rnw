% !Rnw root = ../../main.Rnw
<<setupcmndsdata, cache=FALSE, include=FALSE, results='asis'>>=
...
We chose the web-crawled DECOW16A corpus (TODO RS SCHAEFERBILDHAUER2012, \citealt{SchaeferBildhauer2020}) for all our corpus work.%
...
The only other very large available corpus containing recent German would be the \textit{Deutsches Referenzkorpus} (DeReKo) by the \textit{Institut für Deutsche Sprache} \citep{KupietzEa2010}, but (at least currently) it is not available for scripted access, and it mostly contains newspaper text.
Finally, the COW corpora are based on an improved methodology also used to build the WaCky corpora \citep{BaroniEa2009}, and there are other similar web-derived corpora also actively used by many linguists, such as the SketchEngine corpora \citep{KilgarriffEa2014}.
...
We began by extracting all compounds with a nominal head from the DECOW16A web corpus (TODO RS SCHAEFERBILDHAUER2012, \citealt{SchaeferBildhauer2020}).%
...
In the corpus, nominal compounds come with full structural analyses created automatically using the SMOR finite-state morphological analyser \citep{SchmidEa2004} and extensive pre- and post-processing implemented by the COW creators \citep{SchaeferBildhauer2020}.
...
Table~\ref{tab:freqoverview} is important inasmuch as the numbers reported in it are in line with previous reports based on much smaller corpora such as \citet{WellmannEa1974}, \citet{Kuerschner2005}, or \citet{KrottEa2007} (see also \citealt[9]{Schluecker2012}).
...
The \textit{potential productivity} is the measure of choice for this as it ``gauges the extent to which the market for a category is saturated'' \citep[902, see also 906--907]{Baayen2009}.
...
As shown in (\ref{eq:prod}), the potential productivity $P^p$ is simply the number of the hapax legomena $f_1$ of compounds with $\textrm{N1}_{\lambda}$ divided by the token frequency $f$ of compounds with $\textrm{N1}_{\lambda}$, where $\lambda$ is a variable standing for either PL or NPL.

\begin{equation}\label{eq:prod}
P^p(\textrm{N1}_{\lambda})=\frac{f_1(\textrm{N1}_{\lambda})}{f(\textrm{N1}_{\lambda})}
\end{equation}

The interpretation of $P^p$ is intuitive, as it is $0$ when there are no hapax legomena in the corpus, in which case  $f_1(\textrm{N1}_{\lambda})=0$ (no productivity whatsoever).
Furthermore, it is $1$ if all tokens are hapax legomena, in which case $f_1(\textrm{N1}_{\lambda})=f(\textrm{N1}_{\lambda})$ (maximal productivity).
